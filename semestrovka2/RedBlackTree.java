...
public class RedBlackTree<T extends Comparable<T>> implements Iterable<T>, Iterator<T> {
    ...
    public class Node { // узел дерева
        ...
        private Node _right; // праывй дочерний узел
        ...
        public Node getSuccessor() // следующий по значению узел дерева
        {
            Node temp = null;
            Node node = this;
            if(!node.isRightFree()) {
    ...
    public static <T extends Comparable<T>> void printTree(RedBlackTree<T> tree) { // печатать дерево
        ArrayList<RedBlackTree<T>.Node> nodes = new ArrayList<RedBlackTree<T>.Node>();
        nodes.add(0, tree._root);
        printNodes(tree, nodes);
    }

    private static <T extends Comparable<T>> void printNodes(RedBlackTree<T> tree, ArrayList<RedBlackTree<T>.Node> nodes) { // печатать инф об узле
        int childsCounter = 0;
    ...
    public int add(T o) { // добавление элемента дерева
        int count = 0;
    ...
    public boolean remove(T o) { // удаление элемента дерева
    ...
    public boolean contains(T o) { // проверка на содержание элемента в дереве
    ...
    private Node first() // поиск первого (наименьшего) элемента структуры
    {
        Node node = _root;
